...
export default function Projects() {
  ...
  // Usando a função que criamos ali em cima
  ...
  return (
    <section id="projects" className="w-full py-24 bg-white text-gray-800 px-4 min-h-screen">
      
      <div className="text-center mb-12">
        {/* O Ref fica AQUI no H2 para detectar assim que o título aparecer */}
        <h2 ref={elementRef} className="text-4xl md:text-5xl font-bold mb-4 inline-block min-h-[1.5em]">
          <span className="text-transparent bg-clip-text bg-[linear-gradient(to_right,#9333ea,#db2777,#ea580c,#eab308)]">
            {displayText}
          </span>
          {/* Cursor piscando */}
          ...
          return (
            ...
            >
              ...
              <div className={`absolute inset-0 bg-linear-to-b ${project.colorFrom} ${project.colorTo} flex items-center justify-center transition-opacity duration-500 ${isInactive ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                 <span className="text-white font-bold text-xl tracking-widest uppercase rotate-90 md:whitespace-nowrap">
              ...
              `}>
                ...
                <div className="px-6 py-2 bg-white/20 backdrop-blur rounded-full text-white text-sm font-bold border border-white/30">
                  Ver Detalhes +
                </div>
              </div>
              ...
              <div className={`absolute inset-0 bg-white flex flex-col md:flex-row transition-all duration-700 delay-100 ${isActive ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                ...
                <div className={`w-full md:w-1/2 h-48 md:h-full ${project.imageBg} relative flex items-center justify-center p-4 overflow-hidden`}>
                   <button 
                ...
                <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center bg-gray-50 text-left h-full">
                  <div className="flex gap-2 mb-4 flex-wrap">
                    {project.tags.map(tag => (
                      <span key={tag} className="px-3 py-1 text-xs font-bold rounded-full bg-gray-200 text-gray-600">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <a 
                    ...
                    className={`inline-flex items-center text-lg font-bold ${project.textColor} hover:opacity-80 transition-opacity mt-auto md:mt-0`}
                  >
